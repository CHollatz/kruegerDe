    ...
    return (
        <div className='mt-24 mb-8'>
            <div className='flex flex-row flex-wrap justify-center mx-auto'>
                {images.slice(0, 2).map((image, index) => <CustomImage src={image} onClick={() => { setIsOpen(true); setPhotoIndex(index) }} className='w-1/3' />)}

            </div>
            <div className='flex flex-row flex-wrap justify-center mx-auto'>
                {images.slice(Math.max(images.length - 3, 1)).map((image, index) => <CustomImage src={image} onClick={() => { setIsOpen(true); setPhotoIndex(index + 2) }} />)}
